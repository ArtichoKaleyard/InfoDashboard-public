...
bool DisplayPort::LockFrame(TickType_t timeout_ticks) {
    if (!frame_mutex_) {
        return true;
    }
    return xSemaphoreTake(frame_mutex_, timeout_ticks) == pdTRUE;
}

void DisplayPort::UnlockFrame() {
    if (frame_mutex_) {
        xSemaphoreGive(frame_mutex_);
    }
}

void DisplayPort::RLCD_BeginFrame() {
    LockFrame(portMAX_DELAY);
}
...
int DisplayPort::Width() const {
    return width_;
}

int DisplayPort::Height() const {
    return height_;
}
...
bool DisplayPort::RLCD_CopyPbmBits(uint8_t *dest, size_t dest_len) {
    ...
    const size_t row_bytes = (static_cast<size_t>(width_) + 7U) / 8U;
    const size_t required = row_bytes * static_cast<size_t>(height_);
    if (dest_len < required) {
        return false;
    }
    if (!LockFrame(pdMS_TO_TICKS(500))) {
        return false;
    }
    memset(dest, 0, required);
    for (int y = 0; y < height_; ++y) {
        uint8_t *row = dest + static_cast<size_t>(y) * row_bytes;
        for (int x = 0; x < width_; ++x) {
            if (RLCD_GetPixel(x, y) == ColorBlack) {
                row[x >> 3] |= static_cast<uint8_t>(0x80U >> (x & 0x07));
            }
        }
    }
    UnlockFrame();
...
uint8_t DisplayPort::RLCD_GetPixel(uint16_t x, uint16_t y) const {
    if (x >= width_ || y >= height_) {
        return ColorWhite;
    }
#if (AlgorithmOptimization == 3)
    ...
    if (width_ == 400) {
        ...
        const uint16_t block_y = inv_y / 4;
        const uint32_t index = byte_x * (height_ / 4) + block_y;
        const uint8_t local_x = x % 2;
        const uint8_t local_y = inv_y % 4;
        const uint8_t bit = 7 - (local_y * 2 + local_x);
        return (DispBuffer[index] & (1 << bit)) ? ColorWhite : ColorBlack;
    }
    const uint16_t byte_x = x / 4;
    const uint16_t byte_y = y / 2;
    const uint32_t index = byte_y * (width_ / 4) + byte_x;
    const uint8_t local_x = x % 4;
    const uint8_t local_y = y % 2;
    const uint8_t bit = 7 - (local_x * 2 + local_y);
    return (DispBuffer[index] & (1 << bit)) ? ColorWhite : ColorBlack;
#endif
}
